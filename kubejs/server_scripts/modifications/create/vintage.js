...
ServerEvents.recipes((event) => {
    ...
    // Machinery
    ['turning', 'coiling', 'pressurizing', 'curving', 'hammering', 'laser_cutting', 'centrifugation'].forEach(
        (type) => {
    ...
    [
        ...
        if (smallSpring) {
            vintage
                .coiling(`gtceu:small_${metal}_spring`, `gtceu:${metal}_rod`)
                .id(id(`coiling/small_${metal}_spring`));
        }
        if (doublePlate) {
            // For some reason Item.of(`gtceu:${metal}_plate`, 2) does not work
            vintage
                .hammering(`gtceu:double_${metal}_plate`, [`gtceu:${metal}_plate`, `gtceu:${metal}_plate`])
                .hammerBlows(3)
                .id(id(`hammering/double_${metal}_plate`));
        }
        if (longRod) {
            vintage
                .hammering(`gtceu:long_${metal}_rod`, [`gtceu:${metal}_rod`, `gtceu:${metal}_rod`])
                .hammerBlows(3)
                .id(id(`hammering/long_${metal}_rod`));
        }
    });
    ...
    [
        ...
        vintage
            .vibrating(
                [`gtceu:impure_${mainOre}_dust`, Item.of(`gtceu:${secOre}_dust`).withChance(0.04)],
