...
ServerEvents.recipes((event) => {
    ...
    event.replaceInput(
        ...
        'gtceu:certus_quartz_skystone_alloy_plate'
    );
    event.replaceInput(
        { id: 'ae2:network/cells/fluid_cell_housing' },
        'minecraft:copper_ingot',
        'gtceu:gold_skystone_alloy_plate'
    );
    ...
    const packaging = (tier, type, mat) => {
        ...
        event.recipes.gtceu
            .assembler(id(`${type}_storage_cell_${tier}k`))
            .itemInputs(
                '2x ae2:quartz_glass',
                `ae2:cell_component_${tier}k`,
                '3x minecraft:redstone',
                `3x gtceu:${mat}_skystone_alloy_plate`
            )
        ...
        event.recipes.gtceu
            .canner(id(`${type}_storage_cell_${tier}k`))
        ...
        event.recipes.gtceu
            .packer(id(`${type}_cell_${tier}k_uncrafting`))
    ...
    const craftingStorage = (tier) => {
        ...
        event.recipes.gtceu
            .canner(id(`${tier}k_crafting_storage`))
        ...
        event.recipes.gtceu
            .packer(id(`crafting_storage_${tier}k_uncrafting`))
    ...
    const canner = (output, catalyst, Mega) => {
        ...
        event.recipes.gtceu
            .packer(id(`crafting_${output}_uncrafting`))
    ...
    ['2', '16', '128'].forEach((tier) => {
        ...
        event.recipes.gtceu
            .assembler(id(`spatial_storage_cell_${tier}`))
            .itemInputs(
        ...
        event.recipes.gtceu
            .canner(id(`spatial_storage_cell_${tier}`))
        ...
        event.recipes.gtceu
            .packer(id(`spatial_cell_${tier}_uncrafting`))
